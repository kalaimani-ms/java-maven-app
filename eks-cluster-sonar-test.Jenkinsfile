        ...
        stage ('deployapp to k8s') {
            steps {
                script {
                    ...
                    sh 'envsubst < /var/jenkins_home/workspace/ter-sonar-jenkins_eks_sonar_test/java-maven-app_eks_sonar_test/kubernetes/deployment.yaml | kubectl apply -f - '
                    sh 'envsubst < /var/jenkins_home/workspace/ter-sonar-jenkins_eks_sonar_test/java-maven-app_eks_sonar_test/kubernetes/service.yaml | kubectl apply -f - '
                    sh 'kubectl get pod '
                    sh 'kubectl get all'
                }
            }
